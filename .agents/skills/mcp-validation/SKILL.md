...
4. For pipe-backed tools, ensure the Visual Studio Experimental Instance and `VS MCP Bridge` tool window are active.
5. Capture the first missing triage marker from the runbook rather than guessing from symptoms.
6. Keep MCP stdout protocol-clean; use StdErr, file logs, UI logs, or durable artifacts for diagnostics.
7. Record whether validation used direct MCP stdio, Visual Studio Copilot Agent, Cursor, or a helper script.

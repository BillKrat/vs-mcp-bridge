...
| --- | --- | --- |
| Prior cache-clear attempt report | `docs/blogs/final-rendered-route-verification-after-cache-clear-20260516.md` | Confirms the single HTTP 500 cache-clear attempt and the remaining stale shared marker on all 14 routes. |
| Stale shared widget inspection | `docs/blogs/stale-shared-feature-branch-link-inspection-20260516.md` | Confirms the stale marker matches cached TextBox widget content originally backed by `dbo.be_DataStoreSettings.DataStoreSettingRowId = 26512`; current live DB settings no longer contain the stale marker. |
...
Before another cache-clear attempt, inspect the deployed BlogAI logs or deployed source for the HTTP 500 and confirm whether `/api/settings?action=clearCache` requires an authenticated administrator context, a different anti-forgery/session requirement, or a deployed-only implementation. Once that is documented, perform one known-good cache remediation path and rerun `scripts/blog-publishing/Test-BlogRenderedRoutes.ps1` across the 14 routes.

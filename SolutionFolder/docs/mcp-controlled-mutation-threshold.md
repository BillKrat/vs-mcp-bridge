...
- VS/host diagnostic and proposal tools already documented in `SolutionFolder/docs/ARCHITECTURE.md`
...
The detailed design for that preview-only tool is `SolutionFolder/docs/preview-only-document-update-tool-design.md`.
Practical diff readability improvements are tracked separately in `SolutionFolder/docs/preview-diff-ergonomics-plan.md`; those improvements must remain preview-only and must not add apply/write behavior.

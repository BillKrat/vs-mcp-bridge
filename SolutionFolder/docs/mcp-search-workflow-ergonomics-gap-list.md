...
| Discovering canonical versus historical docs takes time | Agents read `SolutionFolder/docs/blogs/README.md` and prior handoffs, then choose files manually. | Add small curated manifests for common evidence sets, such as BlogAI canonical posts, widget settings, rendered-route reports, and cache/auth source. | Automatic repo-wide source-of-truth inference. |
...
`SolutionFolder/docs/evidence-classification-guidance.md` now defines lightweight evidence categories for future MCP search workflows:
...
4. **Canonical-vs-historical evidence tagging**
   - Status: lightweight guidance added in `SolutionFolder/docs/evidence-classification-guidance.md`.

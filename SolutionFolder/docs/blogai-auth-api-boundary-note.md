...
The boundary should evolve as a Global WebNet auth/API service that BlogAI consumes first, not as one-off BlogAI auth. The broader direction is captured in `SolutionFolder/docs/global-webnet-auth-boundary-direction.md`.
...
The first trust-boundary sketch is `SolutionFolder/docs/blogai-auth-trust-boundary-flow.md`.
The first implementation boundary decision record is `SolutionFolder/docs/adr/0001-blogai-first-auth-boundary.md`.

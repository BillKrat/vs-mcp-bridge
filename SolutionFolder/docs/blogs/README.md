...
Search Interpretation: Hits under `SolutionFolder/docs/blogs/posts/**` indicate current canonical source; hits in preserved exports or reports are supporting evidence, not publishable source.
Currentness: Current as of the latest blog source-of-truth update.

The canonical repository-backed blog format now lives under `SolutionFolder/docs/blogs/posts/`.
...
```text
SolutionFolder/docs/blogs/posts/<slug>/
  post.json
  content.html
```
...
- `SolutionFolder/docs/blogs/posts` is the new source of truth for materialized posts in the repository.
...
- legacy files under `SolutionFolder/docs/blogs/posted` are preserved temporarily as migration input and reference material.
...
- `prepublish-compare-how-stdio-works-in-vs-mcp-bridge-20260516.md` records the fresh read-only pre-publish compare for the stdio post before targeted blocked-row inspection.
...
- `prepublish-inspection-how-stdio-works-in-vs-mcp-bridge-20260516.md` records the targeted read-only inspection that cleared the stdio post for the next guarded single-post review update and documents the intentional `[Page:Stdio]` token.
...
- `publish-review-update-vs-mcp-bridge-blog-series-part-5-20260516.md` records the Part 5 single-post review update, before/after exports, BlogAI reload result, `[Page:Playbook]` token preservation, and rendered-route verification.
- `publish-review-update-vs-mcp-bridge-blog-series-part-6-20260516.md` records the Part 6 single-post review update, before/after exports, BlogAI reload result, `[Page:Evidence]` token preservation, and rendered-route verification.
...
- `publish-review-update-how-stdio-works-in-vs-mcp-bridge-20260516.md` records the stdio single-post review update, before/after exports, BlogAI reload result, `[Page:Stdio]` token preservation, and rendered-route verification.
...
- `final-rendered-route-verification-after-cache-clear-20260516.md` records the controlled cache-clear attempt, its HTTP 500 result, and the follow-up route verification showing post bodies still pass while shared stale widget content remains.
- `blogengine-cache-clear-failure-inspection-20260516.md` inspects the failed `/api/settings?action=clearCache` path, documents the local admin-auth settings API behavior, the remaining local/deployed source mismatch, and the safest remediation options before any retry or app recycle.
...
- live DB/source post: `SolutionFolder/docs/blogs/posts/vs-mcp-bridge-blog-series-part-4/`
- repo publishing trial draft: `SolutionFolder/docs/blogs/posts/vs-mcp-bridge-blog-series-part-4-repo-trial/`
...
```text
SolutionFolder/docs/blogs/source-of-truth/db-export-20260516/
```
...
```text
SolutionFolder/docs/blogs/source-of-truth/widget-settings/
```

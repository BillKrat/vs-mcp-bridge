...
`SolutionFolder/docs/blogs/posts/<slug>/post.json` plus `content.html` is now the repository source of truth for blog posts in `vs-mcp-bridge`.
...
1. Read the repo post from `SolutionFolder/docs/blogs/posts/<slug>/post.json` and `content.html`.

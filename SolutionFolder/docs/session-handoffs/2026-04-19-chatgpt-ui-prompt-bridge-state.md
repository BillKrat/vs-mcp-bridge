...
- `SolutionFolder/scripts/launch-exp.cmd` was fixed to open `VsMcpBridge.slnx` explicitly.
- The solution file had previously been corrupted on disk and was restored from `HEAD`.
- Running `SolutionFolder/scripts/launch-exp.cmd` was verified by process command line to launch:
  - `devenv.exe`
  - `VsMcpBridge.slnx`
  - `/RootSuffix Exp`
  - `/Log`

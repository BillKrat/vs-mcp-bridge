...
| --- | --- | --- | --- |
| `canonical-current` | Current repo source intended to represent the desired state. | `SolutionFolder/docs/blogs/posts/<slug>/post.json`, `SolutionFolder/docs/blogs/posts/<slug>/content.html`, current architecture docs. | Search first when proving whether a marker remains in current source. |
| `operational-handoff` | Session resume context, conclusions, validation status, and next actions. | `SolutionFolder/docs/session-handoffs/*.md`. | Search to recover why work moved a certain direction, but do not treat as source content. |
| `historical-evidence` | Preserved older state, migration input, database export, or comparison baseline. | `SolutionFolder/docs/blogs/source-of-truth/db-export-*`, preserved export folders, legacy reference docs. | Search intentionally when comparing current state to prior state. |
...
| `diagnostic-trace` | Logs, metadata, sequence diagrams, and validation traces from observed tool/runtime workflows. | `SolutionFolder/artifacts/logs/*.log`, `SolutionFolder/artifacts/logs/*.metadata.json`, `SolutionFolder/docs/diagrams/*.mmd`, trace handoffs. | Use to prove what ran, with request ids and observed boundaries. |
...
- `SolutionFolder/docs/blogs/posts/` is `canonical-current` for materialized repo-backed blog posts.
- `SolutionFolder/docs/blogs/source-of-truth/` is preserved source-of-truth evidence, usually `historical-evidence` or `preserved-before-update` depending on the subfolder.
- `SolutionFolder/docs/blogs/source-of-truth/db-export-*` is `historical-evidence` unless a workflow explicitly redefines it as the current database baseline.
- `SolutionFolder/docs/blogs/source-of-truth/widget-settings/*before*` or dated pre-update folders are `preserved-before-update`.
- `SolutionFolder/docs/session-handoffs/` is `operational-handoff`.
- `SolutionFolder/docs/diagrams/` is `diagnostic-trace`.
- `SolutionFolder/artifacts/logs/` is `diagnostic-trace`.
...
- `SolutionFolder/docs/blogs/README.md` as `Canonical/Current Source`
- `SolutionFolder/docs/session-handoffs/2026-05-16-blogai-doc-selection-search-workflow.md` as `Operational Handoff`
- `SolutionFolder/docs/blogs/final-rendered-route-verification-after-cache-clear-20260516.md` as `Rendered Failure Evidence`
- `SolutionFolder/docs/blogs/source-of-truth/widget-settings/datastore-row-26512-20260516/README.md` as `Preserved Before-Update Artifact`
- `SolutionFolder/docs/session-handoffs/2026-05-16-document-selection-validation.md` as `Diagnostic Trace`
...
For BlogAI/blog work, `SolutionFolder/docs/blogs/README.md` should remain the primary index for blog source-of-truth and publishing review workflows.
...
- a stale marker hit in `SolutionFolder/docs/blogs/posts/**` means canonical source needs review
